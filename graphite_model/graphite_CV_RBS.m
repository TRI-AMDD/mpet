function [t,cpcs,clv,plva,plvc,ffvec,vvec,disc] = graphite_comments_RBS(cporos)
...
[t,cpcs] = ode15s(@calcrhs,tr,cpcs0,options,len,ss,asteps,csteps,porvec,fdisc, ...
                        divdisc,nDp,nDm,zp,zm,ioa,ioc,ac,bc,cc,aa,phima,tr);
...
function val = calcrhs(t,cpcs,len,ss,asteps,csteps,porvec,fdisc,divdisc,nDp,nDm,zp, ...
                            zm,ioa,ioc,ac,bc,cc,aa,phima,tr)
                  
val = zeros(2*len+asteps+2*csteps,1);
...
M = sparse(2*len+asteps+2*csteps,2*len+asteps+2*csteps);
