...
poros = 0.65;                        % Porosity
c0 = 1200;                          % Initial electrolyte conc., mol/m^3
...
dim_mcond = 1e-2;                  % Dimensional electronic conductivity
dim_ASRcont = 100;                % Dimensional area specific contact resistance, ohm*m^2

% Particle size distribution
mean = 200e-9;                      % Average particle size, m
stddev = 50e-9;                     % Standard deviation, m
...
alpha = 0.15;                        % Charge transfer coefficient

% Discretization settings
Nx = 25;                            % Number disc. in x direction
numpart = 25;                       % Particles per volume
tsteps = 200;                       % Number disc. in time
ffend = .9;                          % Final filling fraction
...
[t,cpcs]=ode15s(@calcRHS,tr,cpcsinit,options,io,currset,a,alpha,porosvec,numpart,...
                 Nx,disc,tp,zp,zm,nDp,nDm,mcond,tr,epsbeta,ffend,noise, cslow, csup);
...
%in the region is 0, out of region is 1
mumask = 1 - bitand((cslow < cs), (cs < csup)); % 0 when between cslow, csup
% mumask = 1;

width = 0.01;

i1 = bitand((cs > cslow),(cs < cslow + width));
mumask = mumask + i1.*(1-((cs-cslow)/width).^2);
