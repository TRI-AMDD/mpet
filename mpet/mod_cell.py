...
class ModCell(dae.daeModel):
    def __init__(self, config, Name, Parent=None, Description=""):
        ...
        self.c_s = {}
        self.invdRxndetabar = {}
        self.invdRxndetabarG0 = {}
        self.dRxndcl = {}
        self.ffrac = {}
        self.dW_T_phi_sdt = {}
        self.dW_T_phi_lytedt = {}
        self.W_T_c_s = {}
        self.W_T_c_sG0 = {}
        self.G_0 = {}
        self.W_T_c_lyte = {}
        ...
        for trode in trodes:
            ...
            self.dHdc_lyte[trode] = dae.daeVariable(
                "dHdc_lyte_{trode}".format(trode=trode), conc_t, self,
                "Concentration change in the elyte in electrode {trode}".format(trode=trode),
                [self.DmnCell[trode]])
            self.dG0dc_s[trode] = dae.daeVariable(
                "dG0dc_s_{trode}".format(trode=trode), conc_t, self,
                "Concentration change in the elyte in electrode {trode}".format(trode=trode),
                [self.DmnCell[trode]])
            ...
            self.dHdphi_lyte[trode] = dae.daeVariable(
                ...
                [self.DmnCell[trode]])
            self.dHdphi_s[trode] = dae.daeVariable(
                "dHdphi_s_{trode}".format(trode=trode), elec_pot_t, self,
                "Electrostatic potential change in the bulk solid",
                [self.DmnCell[trode]])
        ...
        if config['have_separator']:  # If we have a separator
            ...
            self.dHdphi_lyte[trode] = dae.daeVariable(
                ...
                [self.DmnCell[trode]])
            self.dHdc_lyte[trode] = dae.daeVariable(
                "dHdc_lyte_{trode}".format(trode=trode), elec_pot_t, self,
                "Electric potential in elyte in electrode {trode}".format(trode=trode),
                [self.DmnCell[trode]])
            self.dW_T_phi_lytedt[trode] = dae.daeVariable(
                "dW_T_phi_lytedt_{trode}".format(trode=trode), conc_t, self,
    ...
    def DeclareEquations(self):
        ...
        for trode in trodes:
            for vInd in range(Nvol[trode]):
                ...
                for pInd in range(Npart[trode]):
                    ...
                    RHS1 += -Vj*self.particles[trode][vInd,pInd].cbar()
                eq.Residual = self.R_Vp[trode](vInd) - RHS
                eq1.Residual = self.c_s[trode](vInd) - RHS
                eq = self.CreateEquation(
                    "invdGdeta_trode{trode}vol{vInd}".format(vInd=vInd, trode=trode))
                ...
                RHS1 = 0
                RHS2 = 0
                # added equation to solve for G_0 at each electrode volume
                # sum over particle volumes in given electrode volume
                for pInd in range(Npart[trode]):
                    # The volume of this particular particle
                    Vj = config["psd_vol_FracVol"][trode][vInd,pInd]
                    RHS += -Vj * self.particles[trode][vInd,pInd].dRxndeta()
                    RHS1 += - Vj * self.particles[trode][vInd,pInd].dcbardtG0()
                    RHS2 += - Vj * self.particles[trode][vInd,pInd].dRxndetaG0()
                ...
                eq1.Residual = RHS1
                eq2.Residual = 1/self.invdRxndetabarG0[trode](vInd) - RHS2
                eq = self.CreateEquation(
        ...
        if self.SVsim:
            ...
            dHdphilvec = utils.get_asc_vec(self.dHdphi_lyte, Nvol)
            dHdclvec = utils.get_asc_vec(self.dHdc_lyte, Nvol)
            Rvvec = utils.get_asc_vec(self.R_Vp, Nvol)
            invdRxndetabarvec = utils.get_asc_vec(self.invdRxndetabar, Nvol)
            invdRxndetabarG0vec = utils.get_asc_vec(self.invdRxndetabarG0, Nvol)
            dRxndclvec = utils.get_asc_vec(self.dRxndcl, Nvol)
            ...
            thermFac = elyte_function()[2]

            # get gradient functions
            dphilytedx = utils.central_diff_lyte(self.phi_lyte, Nvol, dx)
            dclytedx = utils.central_diff_lyte(self.c_lyte, Nvol, dx)
            dphisdx = utils.central_diff_bulk(self.phi_bulk, Nvol, dx)
            dcsdx = utils.central_diff_bulk(self.c_s, Nvol, dx)
