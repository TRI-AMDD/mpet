...
def LiFePO42D_aniso(self, c_mat, ybar, T, muR_ref):
    muRtheta = -self.eokT*3.422
    Omga = self.get_trode_param("Omega_a")
    Omgay = self.get_trode_param("Omega_b")
    Omgax = Omga - Omgay
    
    # for allowing muR_ref to be calculated
    if np.size(c_mat) == 1:
        y = c_mat
        muRhomog = self.reg_sln(y, T, Omga)
        ...
        defect_mat = np.random.rand(Nx,Ny)*0.1
        # regular solution
        muR_mat = T*np.log(c_mat/(1- defect_mat - c_mat))
        for x in range(Nx):
            for y in range(Ny):
                if x == 0:
                    muR_mat[x,y] += Omgay*(1- defect_mat [x,y] - 2*c_mat[x,y]) + Omgax*(1- defect_mat [x,y] - c_mat[x+1,y] - c_mat[x+1,y])
                elif x == Nx -1:
                    muR_mat[x,y] += Omgay*(1- defect_mat [x,y] - 2*c_mat[x,y]) + Omgax*(1- defect_mat [x,y] - c_mat[x-1,y] - c_mat[x-1,y])
                else:
                    muR_mat[x,y] += Omgay*(1- defect_mat [x,y] - 2*c_mat[x,y]) + Omgax*(1- defect_mat [x,y] - c_mat[x-1,y] - c_mat[x+1,y])
        ...
        # non-homogeneous
        muR_mat += -self.get_trode_param("kappa_x")*curvx - Omga*curvy
