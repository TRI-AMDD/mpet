...
#        ax.set_ylim((ymin, ymax))
    ...
    if plot_type[0:5] == "cycle":
        ...
        if "current_no_deg" in list(data.keys()):
            current = utils.get_dict_key(data, pfx + 'current_no_deg') / \
                td  # gives us C-rates in /s

        charge_discharge = utils.get_dict_key(data, pfx + "CCCVCPcycle_charge_discharge")
        ind_start_disch, ind_end_disch, ind_start_ch, ind_end_ch = utils.get_negative_sign_change_arrays(
            charge_discharge)
        ...
        for j in range(tot_cycle):
            discharge_cap_temp = integrate.cumtrapz(
                currents[ind_start_disch[j]:ind_end_disch[j]], times[ind_start_disch[j]:ind_end_disch[j]]*td, initial=0)/3600
        ...
        for j in range(tot_cycle):
            charge_cap_temp = integrate.cumtrapz(
                currents[ind_start_ch[j]:ind_end_ch[j]], times[ind_start_ch[j]:ind_end_ch[j]]*td, initial=0)/3600
            # get the total one padded with zeros so we can sum
        ...
        gravimetric_caps = discharge_capacities/(config["P_L"][limtrode] * (
            1-config["poros"][limtrode]) * (config["L"][limtrode]*config['L_ref']) * density) / 1000  # mAh/g
