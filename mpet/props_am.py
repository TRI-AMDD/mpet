...
class muRfuncs():
    ...
    def LIONSIMBA_pos(self, y, ybar, muR_ref, ISfuncs=None):
        ...
        dudt_p   = -0.001 * (0.199521039-0.928373822*y + 1.364550689000003*y**2-0.6115448939999998*y**3);
        dudt_p   = dudt_p/(1-5.661479886999997*y +11.47636191*y**2-9.82431213599998*y**3+3.048755063*y**4);
        #OCV = OCV_ref + dUdT*(T-1)*Tref
        OCV = OCV_ref + dudt_p*(T-1)*Tref
        muR = self.get_muR_from_OCV(OCV, muR_ref)
        actR = None
        return muR, actR
    ...
    def LIONSIMBA_neg(self, y, ybar, muR_ref, ISfuncs=None):
        ...
        OCV = OCV_ref + dUdT*(T-1)*Tref
        #dudt_n = 0.001*(0.005269056 +3.299265709*y-91.79325798*y**2+1004.911008*y**3-5812.278127*y**4 + 19329.7549*y**5 - 37147.8947*y**6 + 38379.18127*y**7-16515.05308*y**8)
        #dudt_n = dudt_n/(1-48.09287227*y+1017.234804*y**2-10481.80419*y**3+59431.3*y**4-195881.6488*y**5+374577.3152*y**6 - 385821.1607*y**7 + 165705.8597*y**8)
        #OCV = OCV_ref + dudt_n*(T-1)*Tref
        muR = self.get_muR_from_OCV(OCV, muR_ref)
        actR = None
        return muR, actR
