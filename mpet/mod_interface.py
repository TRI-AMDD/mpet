...
class InterfaceRegion(dae.daeModel):
    ...
    def DeclareEquations(self):
        ...
        # Concentration: if elyte solid not continuous over interface if liquid continous over interface.
        ...
        if config["elyteModelType"] == "dilute":
            phitmp = np.hstack((self.portInLyte.phi_lyte()+ T * np.log(self.portInLyte.c_lyte()), phivec, phivec[-1]))
        elif config["interfaceModelType"] == "dilute":
            phitmp = np.hstack((self.portInLyte.phi_lyte() - T * np.log(cvec[0]), phivec, phivec[-1]))
        else:
