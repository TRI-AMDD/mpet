...
def case_Rest(step, area, stepIndex, paramfile_header):
    ...
    curr_step_process = np.reshape(np.array([0, None, None, None, None, 1, stepIndex+1]), (1, 7))
...
def case_Charge(step, area, stepIndex, paramfile_header): # add Ends and Limits for current/voltage
    ...
    curr_step_process = np.reshape(np.array([0, None, None, None, None, 0, stepIndex+1]), (1, 7))
    #initialize guess
    if StepMode == 'Current':
       ...
       if step['Limits'] != None:
           if 'Voltage' in step['Limits'].keys():
                #CCCV step
                curr_step_process = np.vstack((curr_step_process, np.array([0, None, None, None, None, 0, stepIndex+1])))
...
def case_ChgFunc(step, area, stepIndex, paramfile_header):
    ...
    if f.subs(t, 0.001) >= 0: # if charge
        ...
        chg_dichg = -1
    #assign CC = 0 as set value and CC charge
    curr_step_process = np.reshape(np.array([str(f), None, None, None, None, chg_dichg, stepIndex+1]), (1, 7))
    #assign CC = 0 as set value and CC charge
    curr_step_process, next_step_index = process_ends(step["Ends"], curr_step_process, area, charge_type = 1) 
    return curr_step_process, next_step_index
